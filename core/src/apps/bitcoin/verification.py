...
class SignatureVerifier:
    def __init__(
        ...
        if not script_sig:
            ...
            elif len(script_pubkey) == 34 and script_pubkey[0] == 0:  # P2WSH
            ...
            elif len(script_pubkey) == 34:  # P2TR
                self.taproot = True
    ...
    def verify(self, digest: bytes) -> None:
        ...
        if self.taproot:
            self.verify_schnorr(digest)
        else:
            self.verify_ecdsa(digest)

    def verify_schnorr(self, digest: bytes) -> None:
        # TODO
